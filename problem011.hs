...
productVertical x ix iy = if length x -iy - 4 > 0 then product [x!!ix!!iy, x!!ix!!(iy+1),x!!ix!!(iy+2), x!!ix!!(iy+3) ] else 0
 

newMax :: (Ord p, Num p) => [[p]] -> Int -> Int -> p -> p
newMax g x y oldHighest = maximum [oldHighest, (productDiagonal g x y), (productHorizontal g x y), (productVertical g x y)]

gridProduct :: (Ord p, Num p) => [[p]] -> p
gridProduct x = gridProduct' x 0 0 0
    where gridProduct' grid ix iy highest | iy +1 == length grid = highest
